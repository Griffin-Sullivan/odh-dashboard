...
```bash
# Run the container
docker run -p 8080:8080 maas-admin
```
...
### Running the Standalone Maas Admin on OpenShift

A complimentary script is provided to build and run MaaS Admin as standalone application
...
```bash
./packages/maas-admin/build.openshift.sh
```

What the script does (high level):

- Detects repo URL and current branch; validates HTTPS fetch URL
- Creates a Docker BuildConfig via `oc new-app` (scoped to `packages/maas-admin`)
- Patches the BuildConfig to use `packages/maas-admin/Dockerfile`
...
```bash
./packages/maas-admin/clean.openshift.sh
```

# OAuth Configuration Guide for maas-admin

This guide explains how to configure OAuth authentication for the `maas-admin` application on OpenShift, including how to set the required environment variables and obtain the OAuth server URL.

## 1. Prerequisites

- Access to an OpenShift cluster with `oc` CLI installed
- Sufficient permissions to create OAuth clients and view routes
- The application route name: `maas-admin`
...
Create an OAuth client for your app. Assuming the route name of your application is `maas-admin`
...
APP_URL=https://$(oc get route maas-admin -ojsonpath={.spec.host})
...
```sh
oc get oauthclient maas-admin -o jsonpath='{.secret}'
````
...
| `OAUTH_CLIENT_ID`          | OAuth client ID (should match your app route) | `maas-admin`                                         |
| `OAUTH_CLIENT_SECRET`      | OAuth client secret from previous step        | `your-client-secret`                                 |
| `OAUTH_REDIRECT_URI`       | Redirect URI for OAuth callback               | `https://maas-admin.apps.example.com/oauth/callback` |
...
```sh
oc set env deployment/maas-admin \
  OAUTH_ENABLED=true \
  OAUTH_CLIENT_ID=maas-admin \
